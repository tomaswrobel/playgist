...
function GistIDE(props: GistIDE.Props) {
	...
	return (
		...
		>
			<NavItem
				href={`https://gist.github.com/${props.gist}`}
				target="_blank"
			>
				View on GitHub
			</NavItem>
			<NavItem href={`/${props.gist}/comments`} target="output">
				Comments
			</NavItem>
			{props.readOnly && (
				<form action="/api/save" method="POST" className="contents">
					...
					<button
						type="submit"
						className="appearance-none bg-transparent text-[#c9d1d9] text-left cursor-pointer hover:bg-[#00000026] no-underline p-2.5 inline-block"
					>
						Fork
					</button>
				</form>
