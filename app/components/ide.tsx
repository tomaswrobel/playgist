	...
	return (
		<div className="flex flex-grow h-screen w-screen font-sans">
			<nav
				...
				className="flex bg-[#272922] flex-col m-0 p-0 overflow-x-hidden overflow-y-auto w-[200px] border-solid border-r border-[#00000026]"
			>
				<NavItem href="https://github.com/tomas-wrobel/playgist">
					PlayGist on GitHub
				</NavItem>
				<NavItem href="/licenses" target="output">
					Credits
				</NavItem>
				<NavDivider />
				{children}
				<a
					className="text-[#c9d1d9] cursor-pointer hover:bg-[#00000026] no-underline p-2.5 inline-block"
					onClick={() => {
						setNewFile("");
					}}
				>
				...
				{Object.keys(files).map(file => (
					<a
						key={"nav-" + file}
						className="text-[#c9d1d9] cursor-pointer hover:bg-[#00000026] no-underline p-2.5 inline-flex gap-2"
						onClick={() => {
					...
					>
						...
						{readOnly || (
							<span
								className="font-[fontello] inline-block antialiased"
								onClick={() => {
									if (current === file) {
										setCurrent(null);
									}
				...
				{newFile === null || (
					<form
						className="text-[#c9d1d9] no-underline p-2.5 inline-flex gap-2 items-center"
						onSubmit={e => {
							e.preventDefault();
			...
			<div className="flex flex-col flex-1 overflow-auto">
				<nav className="flex bg-[#383933] select-none">
					{tabs.map(tab => (
						<div
							key={"tab-" + tab}
							className={`text-[#c9d1d9] p-2 ${
								tab === current ? "bg-[#272822]" : ""
							}`}
					...
					<div className="flex-grow" />
					<a
						className="text-[#c9d1d9] cursor-pointer no-underline p-2 inline-block"
						href="about:blank"
						target="output"
					>
						Run
